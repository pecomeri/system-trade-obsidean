...
def _immediate_volatility_expansion(
    ...
    *,
    ...
    pre_idx = [
        entry_min - pd.Timedelta(minutes=3),
        entry_min - pd.Timedelta(minutes=2),
        entry_min - pd.Timedelta(minutes=1),
    ]
...
def _label_volatility_expansion(trades: pd.DataFrame, atr_m1: pd.Series) -> tuple[list[str], int]:
    labels: list[str] = []
    missing_count = 0
    ...
    for row in trades.itertuples(index=False):
        ...
        label, missing = _immediate_volatility_expansion(atr_m1, entry_time=entry_time)
        if missing:
            missing_count += 1
        labels.append(label)
...
def _write_readme(out_dir: Path, *, notes: list[str], missing_counts: dict[str, int]) -> None:
    lines = [
        "# D016: Distribution change by volatility expansion exclusion (observe)",
        "",
        "## Definitions",
        "- baseline: results/family_D_momentum/D009 (24h, BUY/SELL)",
        "- modified: baseline excluding immediate_volatility_expansion = expanded",
        ...
        "  - ATR period (M1): 14 (TR rolling mean on bid OHLC)",
        "  - pre window: entry_min-3/-2/-1, post window: entry_min/+1/+2",
        "  - missing ATR windows treated as not_expanded",
        ...
        f"- immediate_volatility_expansion missing ATR windows: {missing_counts}",
        "",
...
def main() -> None:
    ...
    out_dir = Path("FX/results/family_D_momentum/D016")
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    for spec in specs:
        ...
        vol_labels, missing_count = _label_volatility_expansion(trades, atr_m1)
        trades["immediate_volatility_expansion"] = vol_labels
        missing_counts[spec.label] = missing_count

        pnl_col = _select_pnl_column(set(trades.columns))

        baseline = trades.copy()
        modified = trades[trades["immediate_volatility_expansion"] != "expanded"].copy()
    ...
    _write_readme(out_dir, notes=notes, missing_counts=missing_counts)
